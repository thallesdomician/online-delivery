/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { OpeningWeekDayWhereUniqueInput } from "../../openingWeekDay/base/OpeningWeekDayWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";
@InputType()
class OpeningWeekDayUpdateManyWithoutStoresInput {
  @Field(() => [OpeningWeekDayWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [OpeningWeekDayWhereUniqueInput],
  })
  connect?: Array<OpeningWeekDayWhereUniqueInput>;

  @Field(() => [OpeningWeekDayWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [OpeningWeekDayWhereUniqueInput],
  })
  disconnect?: Array<OpeningWeekDayWhereUniqueInput>;

  @Field(() => [OpeningWeekDayWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [OpeningWeekDayWhereUniqueInput],
  })
  set?: Array<OpeningWeekDayWhereUniqueInput>;
}
export { OpeningWeekDayUpdateManyWithoutStoresInput };
